...
register_primitive(middle) # required for median(p::Array{Particles})
...
function fit_curve_of_growth(xx, yy)
    ...
    while true
        fit = curve_fit(linear_model, xx, mean.(yy), p0)
...
function calculate_curve_of_growth(multiplet, lines_used, ew_list)
    xx, yy = Float64[], [] # TODO: make yy type stable

    for (λ_c_approx, line_data) in filter(kv -> kv[1] in keys(lines_used), multiplet)
        λ_c_exact = mean(lines_used[λ_c_approx][1])
        push!(xx, line_data.loggf + log10(λ_c_exact))
        push!(yy, log10(mean(ew_list[λ_c_approx])/λ_c_exact))
...
function find_best_multiplet_combo_for_Texc_estimate(multiplet_list, lines_used, ew_list)
    ...
    for i=1:length(multiplet_list), j=i+1:length(multiplet_list)
        ...
        Texc_estimate = calculate_Texc_from_COG_pair((χ₁, xx1, fit1), (χ₂, xx2, fit2))

        let new_error = abs(mean(Texc_estimate) - SUN_REF_TEFF)
            if (new_error < prev_error) || (new_error == prev_error && std(Texc_estimate) < std(Texc))
        ...
        # scatter(xx1, mean.(yy1); yerror=std.(map(x -> ew_list[x.λ_c_approx], multiplet_list[i])))
        # plot!(xx1, [fit1.param[1]*x + fit1.param[2] for x in xx1])
        # scatter!(xx2, mean.(yy2); yerror=std.(map(x -> ew_list[x.λ_c_approx], multiplet_list[j])))
...
Symbol("unweighted_median")
Symbol("weighted_median")
...
function calculate_Texc_from_COG_pair((χ_multiplet1, xx_multiplet1, fit_multiplet1), 
    ...
    # println("1: u(m): $m_multiplet1_err u(b): $b_multiplet1_err")
    # println("2: u(m): $m_multiplet2_err u(b): $b_multiplet2_err")
    ...
    if EXCITATION_TEMP_ALG == :unweighted_median
        y_min_multiplet1, y_max_multiplet1 = 
            (m_multiplet1*xx_multiplet1[begin] + b_multiplet1, 
            m_multiplet1*xx_multiplet1[end] + b_multiplet1)

        y_min_multiplet2, y_max_multiplet2 =
            (m_multiplet2*xx_multiplet2[begin] + b_multiplet2, 
            m_multiplet2*xx_multiplet2[end] + b_multiplet2)
    ...
    elseif EXCITATION_TEMP_ALG == :weighted_median
        y_values = vcat(
            [m_multiplet1*x + b_multiplet1 for x in xx_multiplet1], 
            [m_multiplet2*x + b_multiplet2 for x in xx_multiplet2])
    
        y_median = @unsafe median(y_values) # median is least affected by outliers
